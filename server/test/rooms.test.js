...
test("create, join, configure AI seats, and start a room", () => {
  ...
  room.configureSeat(host, { seat: 2, type: "ai", aiProfile: "sprinter" });
  room.configureSeat(host, { seat: 3, type: "ai", aiProfile: "random" });
  room.startGame(host);
...
test("submitted moves must match legal moves", () => {
  ...
  const host = fakeClient("host");
  const room = manager.createRoom(host, "Alex");
  room.configureSeat(host, { seat: 1, type: "ai", aiProfile: "random" });
  room.configureSeat(host, { seat: 2, type: "ai", aiProfile: "random" });
  room.configureSeat(host, { seat: 3, type: "ai", aiProfile: "random" });
  room.startGame(host);
