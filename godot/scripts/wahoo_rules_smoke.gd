...
static func run() -> Dictionary:
    ...
    for test in [
        ...
        _test_all_base_marbles_exit_eligible_on_roll_6,
        _test_center_entry_from_each_offset,
        ...
        _test_center_exit,
        _test_center_exit_only_on_roll_1,
        _test_center_exit_captures_opponent_on_track,
        _test_cannot_exit_center_onto_own_marble,
        _test_cannot_enter_center_when_occupied_by_own_marble,
        _test_capture_opponent_on_track,
        _test_can_pass_over_opponent_on_track,
        ...
        _test_home_entry_slot0_occupied_blocks_entry,
        _test_other_players_home_entry_is_normal_track,
        _test_win_condition,
...
static func _legal_moves(state, player: int, roll: int) -> Array:
    var rules = WahooRules.new()
    return rules.call("legal_moves", state, player, roll)

static func _apply_move(state, move: Dictionary) -> void:
    var rules = WahooRules.new()
    rules.call("apply_move", state, move)
...
static func _test_all_base_marbles_exit_eligible_on_roll_6() -> Dictionary:
    var name := "all 4 base marbles generate exit_base moves on roll 6"
    var state = WahooState.new_game()
    # All 4 marbles start in base; destination square is empty — expect 4 exit moves
    var moves := _legal_moves(state, 0, 6)
    var exit_moves: Array = moves.filter(func(m): return String(m["kind"]) == "exit_base")
    if exit_moves.size() != 4:
        return _fail(name, "expected 4 exit_base moves, got %d" % exit_moves.size())
    return _ok(name)
...
static func _test_base_exit_capture() -> Dictionary:
    ...
    var moves := _legal_moves(state, 0, 1)
...
static func _test_center_entry_optional() -> Dictionary:
    ...
    var moves := _legal_moves(state, 0, 4)
...
static func _test_center_entry_from_each_offset() -> Dictionary:
    var name := "center entry from each first-6 offset"
    for offset in range(6):
        var state = WahooState.new_game()
        state.marbles[0][0] = WahooState.loc_track(WahooState.base_exit(0) + offset)
        var required_roll := 6 - offset

        var moves := _legal_moves(state, 0, required_roll)
        if _find_move(moves, "enter_center") == null:
            return _fail(name, "offset %d roll %d should allow center" % [offset, required_roll])

        for wrong_roll in range(1, 7):
            if wrong_roll == required_roll:
                continue
            moves = _legal_moves(state, 0, wrong_roll)
            if _find_move(moves, "enter_center") != null:
                return _fail(name, "offset %d roll %d should not allow center" % [offset, wrong_roll])
...
static func _test_center_exit() -> Dictionary:
    ...
    for player in range(WahooState.NUM_PLAYERS):
        ...
        var moves := _legal_moves(state, player, 1)
...
static func _test_center_exit_only_on_roll_1() -> Dictionary:
    var name := "center exits only on roll 1"
    for player in range(WahooState.NUM_PLAYERS):
        var state = WahooState.new_game()
        state.marbles[player][0] = WahooState.loc_center()
        state.center_occupant = [player, 0]
        ...
        for roll in range(2, 7):
            var moves := _legal_moves(state, player, roll)
            if _find_move(moves, "exit_center") != null:
                return _fail(name, "player %d should not exit center on roll %d" % [player, roll])

    return _ok(name)

static func _test_center_exit_captures_opponent_on_track() -> Dictionary:
    var name := "center exit captures opponent on destination"
    var state = WahooState.new_game()
    state.marbles[0][0] = WahooState.loc_center()
    state.center_occupant = [0, 0]
    var dest_idx := WahooState.center_exit_dest(0)
    state.marbles[1][0] = WahooState.loc_track(dest_idx)

    var moves := _legal_moves(state, 0, 1)
    var move = _find_move(moves, "exit_center", WahooState.loc_track(dest_idx))
    if move == null:
        return _fail(name, "missing exit_center move from center")
    if move["captures"] != [1, 0]:
        return _fail(name, "expected capture [1, 0], got %s" % str(move["captures"]))

    _apply_move(state, move)
    if state.center_occupant != null:
        return _fail(name, "center should be empty after exit_center")
    if state.marbles[1][0] != WahooState.loc_base():
        return _fail(name, "captured opponent did not return to base")
    if state.marbles[0][0] != WahooState.loc_track(dest_idx):
        return _fail(name, "exiting marble did not land on expected destination")

    return _ok(name)

static func _test_cannot_exit_center_onto_own_marble() -> Dictionary:
    var name := "cannot exit center onto own marble"
    var state = WahooState.new_game()
    state.marbles[2][0] = WahooState.loc_center()
    state.center_occupant = [2, 0]
    state.marbles[2][1] = WahooState.loc_track(WahooState.center_exit_dest(2))

    var moves := _legal_moves(state, 2, 1)
    if _find_move(moves, "exit_center") != null:
        return _fail(name, "exit_center should be blocked by own marble on destination")

    return _ok(name)

static func _test_cannot_enter_center_when_occupied_by_own_marble() -> Dictionary:
    var name := "cannot enter center occupied by own marble"
    var state = WahooState.new_game()
    state.marbles[0][0] = WahooState.loc_track(WahooState.base_exit(0))
    state.marbles[0][1] = WahooState.loc_center()
    state.center_occupant = [0, 1]

    var moves := _legal_moves(state, 0, 6)
    if _find_move(moves, "enter_center") != null:
        return _fail(name, "enter_center should be blocked when own marble holds center")
...
static func _test_capture_opponent_on_track() -> Dictionary:
    ...
    var moves := _legal_moves(state, 0, 3)
...
static func _test_can_pass_over_opponent_on_track() -> Dictionary:
    var name := "can pass over opponent on track; capture only on landing"
    var state = WahooState.new_game()
    state.marbles[0][0] = WahooState.loc_track(5)
    state.marbles[1][0] = WahooState.loc_track(6)  # opponent one step ahead
    # Roll 3: P0 advances 5→6→7→8, passing through opponent at 6, landing at 8
    var moves := _legal_moves(state, 0, 3)
    var move = _find_move(moves, "advance", WahooState.loc_track(8))
    if move == null:
        return _fail(name, "advance past opponent at intermediate square should be legal")
    if move["captures"] != null:
        return _fail(name, "landing at 8 should not capture opponent at 6")
    return _ok(name)
...
static func _test_wrap_around_track() -> Dictionary:
    ...
    var moves := _legal_moves(state, 1, 3)
...
static func _test_home_entry_slot0_occupied_blocks_entry() -> Dictionary:
    var name := "cannot enter home when entry slot 0 is occupied by own marble"
    var state = WahooState.new_game()
    state.marbles[0][0] = WahooState.loc_track(WahooState.home_entry(0))
    state.marbles[0][1] = WahooState.loc_home(0)  # own marble blocks entry slot
    var moves := _legal_moves(state, 0, 1)
    for move in moves:
        if int(move["marble"]) == 0:
            return _fail(name, "marble 0 at home-entry should have no legal move when slot 0 is full")
    return _ok(name)

static func _test_other_players_home_entry_is_normal_track() -> Dictionary:
    var name := "marble does not turn into home at another player's home-entry square"
    var state = WahooState.new_game()
    # P0 marble passes through P1's home-entry (track 12); P0's own home-entry is 54
    state.marbles[0][0] = WahooState.loc_track(11)
    var moves := _legal_moves(state, 0, 2)
    if _find_move(moves, "advance", WahooState.loc_track(13)) == null:
        return _fail(name, "expected normal advance to track:13 through P1's home-entry at 12")
    if _find_move(moves, "enter_home") != null:
        return _fail(name, "should not enter home at another player's home-entry square")
    return _ok(name)
...
static func _test_exact_landing_on_home_entry_stays_on_track() -> Dictionary:
    ...
    var moves := _legal_moves(state, 3, 2)
    ...
    for roll in [5, 6]:
        moves = _legal_moves(state, 3, roll)
        for move in moves:
            if int(move["marble"]) == 0:
